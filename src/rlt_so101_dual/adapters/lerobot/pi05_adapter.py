...
from rlt_so101_dual.core.interfaces import Observation, VLAOutput
from rlt_so101_dual.core.utils import postprocess_prefix_tokens
from rlt_so101_dual.core.vla_adapter import VLAAdapter
...
class Pi05VLAAdapter(VLAAdapter):
    def __init__(
        ...
        actual_action_dim: int = sc.ACTION_DIM,
        actual_proprio_dim: int = sc.PROPRIO_DIM,
        ...
        self.camera_name_map = camera_name_map or dict(sc.PI05_CAMERA_MAP)
        # Only the slots this robot actually fills. A slot left out of
        # camera_order costs nothing; a slot left in but unfilled is padded
        # with a blank image that still consumes a full camera's worth of
        # prefix tokens (256 at 224px / patch 14).
        self.camera_order = sorted(
            set(self.camera_name_map.values()),
            key=lambda k: (sc.PI05_CAMERA_ORDER.index(k) if k in sc.PI05_CAMERA_ORDER else 99, k),
        )

        pi05_config = PI05Config(
            device=device,
            dtype=dtype,
            chunk_size=sc.VLA_HORIZON,
            n_action_steps=sc.VLA_HORIZON,
            max_action_dim=sc.PI05_MAX_DIM,
            max_state_dim=sc.PI05_MAX_DIM,
        ...
        pi05_config.input_features["observation.state"] = PolicyFeature(
            type=FeatureType.STATE, shape=(sc.PI05_MAX_DIM,)
        )
        pi05_config.output_features = {
            "action": PolicyFeature(type=FeatureType.ACTION, shape=(sc.PI05_MAX_DIM,)),
        }
    ...
    def _prepare_language_tokens(self, obs: Observation) -> tuple[torch.Tensor, torch.Tensor]:
        device = self.model_device
        # Slicing to actual_proprio_dim is a no-op on a tensor that is already
        # narrower, so a robot-vs-config mismatch would pad garbage to 32 and
        # report success. Check the width before trusting the slice.
        sc.check_last_dim(
            "observation.state fed to pi0.5",
            obs.proprio,
            self.actual_proprio_dim,
            hint="The dataset's observation.state does not match "
            "actual_proprio_dim. Check the robot in the manifest against "
            "rlt_so101_dual.core.shape_contract.",
        )
