...
from rlt_so101_dual.adapters.lerobot.record.acp_tags import build_acp_tagged_task
...
def _set_so_leader_manual_control(teleop: Teleoperator, enabled: bool) -> None:
    ...
    manual_control_enabled = getattr(teleop, "_rlt_so101_dual_manual_control_enabled", True)

    def _bus_op(op_name: str, fn) -> None:
        # Feetech "Input voltage error" / packet errors often show up as
        # RuntimeError (not ConnectionError) right after a heavy follower
        # go-home. Retry a few times before giving up so one glitch doesn't
        # abort the whole online session after s/f.
        ...
        for attempt in range(1, 4):
            try:
                ...
                _raise_so_leader_connection_error(teleop, op_name, error)
            except RuntimeError as error:
                last_err = error
                logging.warning(
                    "Leader bus %s failed (attempt %d/3): %s", op_name, attempt, error
                )
                time.sleep(0.4 * attempt)
        raise RuntimeError(
            f"Leader bus {op_name} failed after retries (often Feetech input-voltage / "
            f"brownout after go-home). Check 12V supply and leader USB; then re-run. "
            f"Last error: {last_err}"
        ) from last_err

    if enabled:
        if not manual_control_enabled:
            _bus_op("enabling leader manual control", teleop.bus.disable_torque)
            teleop._rlt_so101_dual_manual_control_enabled = True
        return

    if manual_control_enabled:
        _bus_op("disabling leader manual control", teleop.bus.enable_torque)
        teleop._rlt_so101_dual_manual_control_enabled = False
...
def ramp_teleop_to_action(
    teleop: Teleoperator,
    target: RobotAction,
    *,
    steps: int,
    fps: float,
    reason: str = "leader sync",
) -> None:
    """Smoothly drive leader arms to ``target`` (same joint keys as the follower).

    After follower go-home, leaders often still sit at the last autonomous /
    mid-episode pose. Opening the reset teleop window then makes followers
    chase those stale leader angles (80°+ snaps, clamp spam, brownouts).
    Call this *before* unlocking leaders for backdrive.
    """
    import logging
    import time
    ...
    if steps < 1:
        steps = 1
    joint_keys = [k for k, v in target.items() if k.endswith(".pos") and isinstance(v, (int, float))]
    if not joint_keys:
        logging.warning("%s: no joint targets; skipping leader ramp.", reason)
        return

    try:
        start = teleop.get_action()
    except Exception:
        logging.exception("%s: could not read leader pose; sending final target only.", reason)
        send_teleop_feedback(teleop, {k: float(target[k]) for k in joint_keys})
        time.sleep(0.5)
        return

    logging.info(
        "%s: ramping %d leader joints over %.1fs (%d steps).",
        reason,
        len(joint_keys),
        steps / max(fps, 1e-6),
        steps,
    ...
    for i in range(1, steps + 1):
        ...
        alpha = i / steps
        blended = {
            name: float(start.get(name, target[name]))
            + (float(target[name]) - float(start.get(name, target[name]))) * alpha
            for name in joint_keys
        }
        try:
            send_teleop_feedback(teleop, blended)
        except Exception:
            logging.exception("%s failed at step %d/%d; leaving leaders where they are.", reason, i, steps)
            return
        precise_sleep(max(1.0 / max(fps, 1e-6) - (time.perf_counter() - step_t), 0.0))
    # Hold final pose briefly so PID settles before torque is released for reset.
    time.sleep(0.4)
