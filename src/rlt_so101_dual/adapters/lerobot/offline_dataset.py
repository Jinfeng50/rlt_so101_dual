...
from rlt_so101_dual.adapters.lerobot.demo_loader import RLTDemoDataset, rlt_demo_collate
from rlt_so101_dual.core.interfaces import ChunkTransition, Observation
...
def load_transition_cache(
    ...
    chunk_length: int = sc.CHUNK_LENGTH, strict: bool = True,
) -> ReplayBuffer:
    """Load a chunk-transition cache into a replay buffer.

    A cache is a bare list of tensors with no dimension metadata, so a cache
    built for a different robot loads without complaint and only surfaces
    once a batch reaches the networks -- or not at all, if the shapes happen
    to broadcast. Measure the dimensions and compare them against the
    contract. `strict=False` downgrades the mismatch to a warning for the
    deliberate case of inspecting someone else's cache.
    """
    ...
    if data:
        first = buf.buffer[0]
        # ChunkTransition keeps ref_chunk unbatched as (C, action_dim), so the
        # dimensions read straight off the shapes.
        action_dim = int(first.ref_chunk.shape[-1])
        state_width = int(first.state_vec.shape[-1])
        # Synthetic fixtures use state vectors far narrower than one RL token,
        # so there is no meaningful proprio width to check on those.
        proprio_dim = state_width - sc.RL_TOKEN_DIM if state_width > sc.RL_TOKEN_DIM else None

        mismatch = action_dim != sc.ACTION_DIM or (
            proprio_dim is not None and proprio_dim != sc.PROPRIO_DIM
        )
        if mismatch:
            message = (
                f"{path} holds transitions for action_dim={action_dim}"
                + (f", proprio_dim={proprio_dim}" if proprio_dim is not None else "")
                + f"; this project's contract is action_dim={sc.ACTION_DIM}, "
                f"proprio_dim={sc.PROPRIO_DIM}. The cache was built for a different robot."
            )
            if strict:
