...
from rlt_so101_dual.core.interfaces import Observation, VLAOutput, ChunkTransition
...
def test_config_defaults():
    ...
    cfg = RLTConfig()
    assert cfg.action_dim == sc.ACTION_DIM == 12
    assert cfg.proprio_dim == sc.PROPRIO_DIM == 12
    assert cfg.cameras == sc.CAMERA_KEYS == ["left_wrist", "right_wrist", "right_front"]
    assert cfg.control_hz == sc.FPS == 30
...
def test_config_default_matches_the_contract():
    """default() used to read a shipped base.yaml that still carried the
    upstream bimanual 14-DoF numbers, and this test pinned them."""
    from rlt_so101_dual.core import shape_contract as sc

    cfg = RLTConfig.default()
    assert cfg.action_dim == sc.ACTION_DIM
    assert cfg.proprio_dim == sc.PROPRIO_DIM
    assert cfg.cameras == sc.CAMERA_KEYS
    assert cfg == RLTConfig()
